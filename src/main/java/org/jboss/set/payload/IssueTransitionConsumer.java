...
public class IssueTransitionConsumer extends AbstractIssueConsumer {
    ...
    private static final String COMPONENT_UPGRADE_MESSAGE = "This component upgrade has been incorporated in manifest[*].\n\n[*] %s";
    private static final String INCORPORATED_ISSUE_MESSAGE = "This issue has been incorporated in manifest[*] via component upgrade %s.\n\n[*] %s";
    ...
    public void incorporatedIssue(Issue issue, Issue componentUpgrade, String manifestReference) {
        String comment = String.format(INCORPORATED_ISSUE_MESSAGE, componentUpgrade.getKey(), manifestReference);
        if (!hasLabel(issue, LABEL)) {
            issueClient.addComment(issue, comment);
            issueClient.updateIssue(issue, fixVersions, LABEL);
        } else {
